...
export async function GET(req) {
  ...
  for (const app of apps) {
    ...
    rows.push({
      ...
      state: clean(app?.state || app?.homeState || app?.applicantState || ''),
      refCode: clean(app?.refCode || app?.referral_code || ''),
      referredByRaw: clean(app?.referralName || app?.referredByName || app?.referred_by || ''),
      firstName: clean(app?.firstName || app?.first_name || ''),
      lastName: clean(app?.lastName || app?.last_name || ''),
      licensed: Boolean(member?.licensed ?? boolLicensed(app?.isLicensed)),
      licensedStatus: clean(app?.isLicensed || ''),
      appStatus,
      originalReferrer,
      effectiveReferrer,
