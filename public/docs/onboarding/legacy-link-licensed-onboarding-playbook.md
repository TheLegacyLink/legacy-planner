# Welcome to The Legacy Link — Licensed Agent Onboarding

Hey, welcome. You're licensed and you're here — that means you're already ahead. Now let's get you set up the right way.

Read this once, then follow each step in order. Don't skip. Each step matters.

---

## Step 1 — Sign Your Agreement (Do This First, Right Now)

Before you do anything else, you need to sign your Independent Contractor Agreement. This is a one-time thing. It takes about 5 minutes.

Here's exactly what happens:

**1. Go to your back office login:**
**innercirclelink.com/start**

**2. Enter your email address.**
A 6-digit code will be sent to you. Enter it to get in.

**3. You'll see a full-screen signing flow. It has 3 parts:**

**Part 1 — The ICA (Independent Contractor Agreement)**
This is your official agreement with The Legacy Link. Scroll through and read it. When you're done, check the box that says you've read and agree, then click Continue.

**Part 2 — Compliance & Coverage Addendum**
A shorter document covering insurance policies, optional coverage terms, and your rights. Read it, check the box, click Continue.

**Part 3 — Suitability Questions + Policy Election**
You'll answer 5 simple questions. They help us figure out if a company-funded life insurance policy makes sense for you. Be honest — there's no wrong answer.

The questions are:
- Do you have any life insurance right now?
- Does anyone rely on you financially? (family, spouse, kids, business partner)
- Do you have financial obligations that would hurt your family if something happened to you? (rent, debt, bills)
- Would your family struggle financially if you passed away unexpectedly?
- Do you understand what life insurance does and how it protects people?

After you answer, one of two things will happen:

✅ **If you're suitable** — you'll see a checkbox to elect a company-funded policy. Check it if you want it. It's optional. If you'd rather skip it, there's an opt-out box too.

⚠️ **If you're not suitable right now** — the policy box will be grayed out. You'll check a box saying you understand and want to move forward anyway. You're still 100% part of Legacy Link — the policy is always optional.

**4. Type your full legal name to sign and click "Sign & Complete."**

Done. Kimora and your upline will be notified to countersign.

---

## Step 2 — Complete Your Contracting

You need to be contracted to get paid. There are two parts:

**Part A — Pinnacle Group** (Mutual of Omaha, Transamerica, Foresters, American National):
**https://surelc.surancebay.com/producer/?gaId=190**

**Part B — InvestaLink** (F&G + National Life Group):
[Click here to complete your contracting with F&G and National Life Group](https://accounts.surancebay.com/oauth/authorize?redirect_uri=https:%2F%2Fsurelc.surancebay.com%2Fproducer%2Foauth%3FreturnUrl%3D%252Fprofile%252Fcontact-info%253FgaId%253D168%2526gaId%253D168%2526branch%253DInvestaLink%2526branchVisible%253Dtrue%2526branchEditable%253Dfalse%2526branchRequired%253Dtrue%2526autoAdd%253Dfalse%2526requestMethod%253DGET&gaId=168&client_id=surecrmweb&response_type=code)

**Part C — Watch the contracting tutorial** so you know what you're doing:
**https://www.loom.com/share/79354f8de2334697ba53cc5b0ff80c86**

If you're contracting with National Life Group, expect a follow-up email within 48 hours to complete one extra form. Don't ignore it.

---

## Step 3 — Get Your E&O Active

E&O is required before you write any policies. Get it here:
**https://buy.stripe.com/dRm6oH25qe7521Cg4b3ZK0m**

There's a 90-day free period before billing starts. But get it now so it doesn't slow you down later.

---

## Step 4 — Complete Your Product Training

Log into each carrier platform and complete their required training. You need to be able to explain the products clearly before you're in front of a client. This isn't optional.

Carriers: National Life Group, F&G, Mutual of Omaha (and others based on your contracting).

---

## Step 5 — Set Up Your CRM, Calendar, and Dialer

Your tools don't work if they're not set up. Watch the setup video:
**https://youtu.be/u3_fIRH8c0w**

Keep your CRM notes clean and updated the same day, every day.

---

## Step 6 — Script Roleplay Certification

Work with your upline or trainer to go through your scripts until your delivery is clean and consistent. This is how you get confident fast.

---

## Step 7 — Watch "Whatever It Takes"

Watch this video and leave a comment:
**https://youtu.be/SVvU9SvCH9o?si=nzgjgEa7DfGQlxmX**

This is required. It's part of our culture. Watch it with intention.

---

## Lead Access — How It Works

Once all your steps are done, leads start coming in.

- **Full Sponsorship Path** (suitability confirmed + policy opted in): 60 fresh leads + full JumpStart access
- **Alternate Path** (opted out or not suitable): 20 leads, drip delivery, continued support

If you're working leads consistently but haven't closed yet, we may add more to support your momentum.

---

## Daily Standards (Non-Negotiable)

- Work new leads every single day
- Follow up warm leads every single day
- Use your script and keep improving with feedback
- CRM notes updated same day — no exceptions

---

## Need Help?

Use the Help / Messages button inside your back office to reach your upline directly.
Response time: 1 business day (Monday–Friday). Weekends don't count.

If your upline doesn't respond by end of next business day, the issue gets escalated internally.

---

**You're licensed. You're here. Now execute.**
— The Legacy Link Team
