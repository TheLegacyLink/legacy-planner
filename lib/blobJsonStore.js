...
export async function loadJsonFile(pathname, fallback = null) {
  ...
  try {
    ...
    // We paginate through ALL versions and sort by uploadedAt to reliably get the newest.
    ...
    const newestVersion = allVersionBlobs
      .sort((a, b) => new Date(b.uploadedAt || b.uploaded_at || 0).getTime() - new Date(a.uploadedAt || a.uploaded_at || 0).getTime())[0];
